...
void pojavSetWindowHint(int hint, int value) {
    if (hint == GLFW_CLIENT_API) {
        ...
        switch (value) {
            case 1:
            case 2:
                // Legacy GL (<=1.16.5): gl4es exports OpenGL 2.1
                setenv("POJAV_RENDERER", RENDERER_NAME_GL4ES, 1);
                JNI_LWJGL_changeRenderer(RENDERER_NAME_GL4ES);
                break;
            default:
                // Modern core-profile GL (1.17+): use Zink (Mesa->Vulkan->MoltenVK).
                // Upstream pointed this at MobileGlues, whose dylib is never built
                // in this fork -> dlopen fails -> black screen. Zink actually ships.
